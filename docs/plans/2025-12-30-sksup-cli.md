...
- Decision: Defer `skillssupply-nkt` (package.toml schema + extraction) and exclude it from this scope.
...
- `sk pkg` add flow (local path, alias) writes `package.toml`; remove flow deletes selected package.
