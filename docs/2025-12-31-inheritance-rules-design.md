...
Current `discoverManifests` walks up from CWD to home directory (or filesystem root), then appends `~/.sk/agents.toml`. This causes:
...
3. **Confusing "global" behavior**: `~/.sk/agents.toml` is part of inheritance instead of being user-level only
...
|-------|-----------|----------------|
| Local | Walk up to find `agents.toml` (project root) | `{project_root}/.{agent}/skills/` |
| Global | `~/.sk/agents.toml` only | `~/.{agent}/skills/` |

**Key principles:**
- **Manifest = boundary**: First `agents.toml` found walking up is project root
...
findProjectRoot(startDir):
  1. Walk up from startDir looking for agents.toml
  2. Stop conditions (in order checked):
     - Found agents.toml → return that directory
...
discoverManifest(startDir):
  ...
  2. If projectRoot: return projectRoot/agents.toml
  3. Else: return null
```

**Key behaviors**:
- No inheritance, no merging, single manifest
- Home directory is an upper bound (won't find agents.toml in /usr/share/something)
- Git is NOT used as a boundary marker
- **First found wins**: In nested scenarios (monorepo), closest agents.toml is the project root. Parent manifests are ignored.

### Global Scope (`--global` flag)

```
discoverGlobalManifest():
  - Return ~/.sk/agents.toml if exists
...
```
~/projects/myapp/
├── agents.toml        ← contains "superpowers"
└── src/
```

`sk sync` from `~/projects/myapp/`:
- Discovery: Found `agents.toml` in CWD
...
├── agents.toml        ← contains "superpowers"
└── src/
    └── lib/            ← no agents.toml here
```

`sk sync` from `~/projects/myapp/src/lib/`:
- Discovery: Walk up, find `agents.toml` at `myapp/`
- Project root: `~/projects/myapp/`
- **Blocking prompt** (single prompt with warning + choices):
  ```
  ⚠ Found agents.toml at ~/projects/myapp/
  ...
  [n] Create new agents.toml here instead
  [q] Cancel
  ```
- If `c`: Install to `~/projects/myapp/.claude/skills/`
- If `n`: Create `~/projects/myapp/src/lib/agents.toml`, install to `~/projects/myapp/src/lib/.claude/skills/`

### Example 3: Nested agents.toml (monorepo)

```
~/projects/monorepo/
├── agents.toml        ← contains "monorepo-tools"
└── packages/
    └── sub-app/
        ├── agents.toml    ← contains "sub-app-tools"
        └── src/
```

`sk sync` from `~/projects/monorepo/packages/sub-app/`:
- Discovery: Found `agents.toml` in CWD (first found wins)
- Project root: `~/projects/monorepo/packages/sub-app/`
- Manifest: `sub-app/agents.toml` only (monorepo's is ignored)
- Install to: `~/projects/monorepo/packages/sub-app/.claude/skills/`

### Example 4: No agents.toml anywhere

```
~/random-folder/
└── subfolder/          ← no agents.toml anywhere up to ~
```

`sk sync` from `~/random-folder/subfolder/`:
- Discovery: Walk up, hit home dir, no agents.toml found
- **Prompt**: "No agents.toml found. Create one here?"
- If yes: Create `~/random-folder/subfolder/agents.toml`
...
~/.sk/
└── agents.toml        ← contains "global-helpers"
```

`sk sync --global` from anywhere:
- Ignores CWD entirely
- Manifest: `~/.sk/agents.toml` only
...
- **Blocking prompt** (single prompt with warning + choices):
  ```
  ⚠ Found agents.toml at ~/projects/myapp/
    This will modify ~/projects/myapp/agents.toml
  ...
  [q] Cancel
  ```
- If `c`: Modifies `~/projects/myapp/agents.toml` (adds superpowers)
- If `n`: Creates `~/projects/myapp/src/lib/agents.toml` with superpowers
...
### Case 1: agents.toml found in CWD
- Use it as project root
- Install to `CWD/.{agent}/skills/`

### Case 2: agents.toml found in PARENT (not CWD)
...
Claude/codex look for skills relative to where they're launched, not where agents.toml lives. If user:
...
### Case 3: No agents.toml found anywhere
- Prompt: "No agents.toml found. Create one here? [y/N]"
...
| `sk sync --global` | `~/.sk/agents.toml` | Prompt to create |
| `sk pkg add <dep>` | Project root | Case 2 or 3 above |
| `sk pkg add --global <dep>` | `~/.sk/agents.toml` | Prompt to create |
| `sk pkg remove <dep>` | Project root | Error if no project found |
| `sk pkg remove --global <dep>` | `~/.sk/agents.toml` | Error if missing |
...
sk sync:
  ...
  2. If projectRoot found in PARENT (not CWD):
       ...
       - If 'n': create agents.toml at CWD, use CWD as projectRoot
       - If 'q': exit
  3. If no projectRoot found:
       - Prompt: "No agents.toml found. Create one here? [y/N]"
       - Default is No (safe default)
       - If yes: create agents.toml at CWD
  4. manifest = loadManifest(projectRoot/agents.toml)
  5. For each enabled agent:
       installTo(projectRoot/.{agent}/skills/, manifest.dependencies)

sk sync --global:
  1. manifest = discoverGlobalManifest()
  2. If not exists → prompt to create ~/.sk/agents.toml
...
```
sk pkg add <dep>:
  - Find project root (walk up to agents.toml)
  - If projectRoot found in PARENT (not CWD):
      ...
      - If 'n': create agents.toml at CWD with the dependency
      - If 'q': exit
  - If projectRoot in CWD: modify that agents.toml
...
sk pkg add --init <dep>:
  - If no project found: create agents.toml at CWD (no prompt)
...
sk pkg add --global <dep>:
  - Target: ~/.sk/agents.toml
  - If missing: prompt "Create ~/.sk/agents.toml? [y/N]" (consistent with local behavior)

sk pkg remove <dep>:
  - Find project root, modify that agents.toml
...
```
sk init:
  - Create agents.toml in CWD with:
  ...
  - Error if agents.toml already exists in CWD

sk init --global:
  - Create ~/.sk/agents.toml with:
    - [agents] section (prompt with auto-detected options)
    - Empty [dependencies] section
  - Error if ~/.sk/agents.toml already exists
...
async function findProjectRoot(startDir: string): Promise<string | null> {
  // Walk up from startDir looking for agents.toml
  // Stop at ~ or /
  // Return the directory containing agents.toml, or null if not found
}
```
...
- Add blocking confirmation when agents.toml found in parent (not CWD)
- Add prompt to create agents.toml when none found
...
| `agents.toml` is a directory | Error: "agents.toml exists but is not a file" |
| Home directory has `agents.toml` | Valid project root (not treated specially) |
| Running from `~` itself | Check for `~/agents.toml`, stop there |
